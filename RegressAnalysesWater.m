...
EnsemblesNames = [{'Mean'},{'Median'},{'PCA_weighting'},{'Median_among'},{'RegressAmong'},...
...
for ensem = 11:12%1:length(EnsemblesNames)
    load('TheWaterDataperKM.mat')
    Percentiles(:,1)=prctile(EnsemblesWater.SEMamongEnsembles,[0,5,10,25,50,75,90,95,100]);
Percentiles(:,2)=prctile(EnsemblesWater.SEM_among_Models,[0,5,10,25,50,75,90,95,100]); %#ok<NASGU>
    ...
    clearvars -except Results EnsemblesNames VariableNames VariList run_max Residuals Mdls Percentiles
end
save('ResultsWaterComparison','Results','Residuals','Mdls','Percentiles');
end
